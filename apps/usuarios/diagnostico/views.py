...
from .models import Diagnostico, Historial
from ..usuarios.models import Usuario, Estudiante, PerfilEstudiante
...
def vista_pregunta(request, numero):
    ...
    if not sesion_id:
        sesion = Historial.objects.create(
            usuario=request.user,
...
def vista_historial(request):
    ...
    sesiones = Historial.objects.filter(
        usuario=request.user,
        completado=True,
    ).order_by("-fecha_inicio")
